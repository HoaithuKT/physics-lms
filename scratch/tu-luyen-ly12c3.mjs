/**
 * Dựng module "Bài tập tự luyện" cho bảy bài của chương 3 Vật lí 12.
 *
 * Hạn ngạch lớp 12 (quy ước đã chốt bên Toán, đề tốt nghiệp Lý cùng ba phần nên dùng lại):
 *   20 NLC + 4 DS + 6 TLN = 30 câu, KHÔNG có tự luận.
 *   NLC chỉ mức 1-2, tỉ lệ 40/60 · DS và TLN chia 25/50/25 cho mức 2/3/4.
 *   Kho chương này KHÔNG có câu mức 4 nên phần 25% mức 4 được bù bằng mức 3.
 *
 * Bài 20 là bài tập tổng hợp, không có câu riêng trong kho → rút từ CẢ CHƯƠNG, xoay vòng
 * theo bài, chỉ lấy câu chưa dùng ở đâu.
 *
 *   node scratch/tu-luyen-ly12c3.mjs        -> thử
 *   node scratch/tu-luyen-ly12c3.mjs ghi    -> ghi thật
 */
...
for (const t of [14, 15, 16, 17, 18, 19, 20]) {
  const tep = `scratch/lt-ly12c3-bai${t}-day-du.md`;
...
const BAI = [
  { ten: 'Bài 14. Từ trường', lesson: 'b94d44f6-7a3f-470a-ba59-d6f14673cb7f' },
  { ten: 'Bài 15. Lực từ tác dụng lên dây dẫn mang dòng điện. Cảm ứng từ', lesson: '9d3c7d0b-4f8c-420b-8a0a-4b7c12ac5c20' },
  { ten: 'Bài 16. Từ thông. Hiện tượng cảm ứng điện từ', lesson: '28179ddb-d2b9-4ef4-82ac-428aefd449a8' },
  { ten: 'Bài 17. Máy phát điện xoay chiều', lesson: '5f0d7dab-8e12-4421-a23b-849a23b66ecf' },
  { ten: 'Bài 18. Ứng dụng hiện tượng cảm ứng điện từ', lesson: 'b7d9bd4e-4ef9-4f86-81e8-760e39c2832e' },
  { ten: 'Bài 19. Điện từ trường. Mô hình sóng điện từ', lesson: '95cba988-5837-496d-b803-3eff9b84b705' },
  { ten: 'Bài 20. Bài tập về từ trường', lesson: 'cd54a43a-f9f8-4a2b-8c3d-e7fef63e9c56', caChuong: true },
];

/* Câu đã vào tự luyện của các bài trước - Bài 20 rút cả chương nên phải né. */
const daVaoTuLuyen = new Set();
...
for (const b of BAI) {
  ...
  /* Với Bài 20, "dạng" xoay vòng chính là BÀI (để câu trải đều sáu bài), bài ít câu trước. */
  const thuocBai = (q) => b.caChuong ? true : q.lesson === b.ten;
  const nhom = (q) => b.caChuong ? (q.lesson || '?') : q.math_form;
  const dsDang = [...new Set(kho.filter(thuocBai).map(nhom))]
    .sort((x, y) => kho.filter(q => thuocBai(q) && nhom(q) === x).length
                  - kho.filter(q => thuocBai(q) && nhom(q) === y).length);

  const dungDuoc = (q, loai) => thuocBai(q) && q.question_type === loai
    && !dung.has(q.id) && !daDungLyThuyet.has(q.id) && !daVaoTuLuyen.has(q.id)
  ...
  for (const [loai, muc, can] of HAN_NGACH) {
    ...
    while (lay < can && vong < 50) {
      ...
      for (const dang of dsDang) {
        if (lay >= can) break;
        const ung = kho.filter(q => dungDuoc(q, loai) && String(q.difficulty) === muc && nhom(q) === dang)
  ...
  for (const q of chonRa) daVaoTuLuyen.add(q.id);
  ...
  if (b.caChuong) console.log(`   theo bài: ${[...new Set(chonRa.map(q => q.lesson))].map(l => `${String(l).slice(0, 6)}:${chonRa.filter(q => q.lesson === l).length}`).join(' ')}`);
  ...
  if (cu) {
    ...
    const { error } = await sb.from('lesson_modules').insert({
      lesson_id: b.lesson, title: TEN_MODULE, type: 'practice', content_markdown: md, order_index: viTri,
    });
