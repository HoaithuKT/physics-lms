/**
 * Logic dựng prompt và đọc kết quả cho tính năng AI gán Dạng vật lý + Mức độ.
 *
...
export interface BuildDetectFormsPromptParams {
  ...
  /** Dạng vật lý đã có, trong đúng phạm vi Chương/Bài đang soạn */
  formsToUse: string[];
  /** Nhãn "Vật lý tổng hợp" (hoặc tương đương) đang dùng trong hệ thống */
...
export function buildDetectFormsPrompt({
  ...
  const formListStr = formsToUse.length > 0
    ...
    : '(Chưa có Dạng vật lý nào trong phạm vi Chương/Bài này)';
...
[
  ...
  return `Bạn là một chuyên gia phân loại đề Vật lý, đang giúp giáo viên gắn "Dạng vật lý" và "Mức độ" cho từng câu hỏi trước khi lưu vào Ngân hàng câu hỏi.

DANH SÁCH DẠNG VẬT LÝ ĐÃ CÓ SẴN TRONG NGÂN HÀNG (chỉ trong phạm vi Chương/Bài đang soạn):
${formListStr}

CÂU HỎI CẦN PHÂN LOẠI:
${questionsBlock}

QUY TẮC VỀ DẠNG VẬT LÝ:
1. Với câu KHÔNG phải Đúng/Sai: chọn 1 Dạng vật lý khớp nhất trong danh sách có sẵn. Nếu KHÔNG có dạng nào phù hợp, được phép TỰ ĐỀ XUẤT một tên Dạng vật lý mới, ngắn gọn, đúng văn phong các dạng đã có (ví dụ "Tìm khoảng đồng biến của hàm số", không viết câu đầy đủ, không có dấu chấm cuối).
2. Với câu Đúng/Sai 4 mệnh đề: xét TỪNG mệnh đề a, b, c, d riêng biệt xem thuộc dạng vật lý nào.
   - Nếu CẢ 4 mệnh đề cùng thuộc một Dạng vật lý -> trả về đúng dạng đó (isNew=false nếu dạng đã có sẵn).
   - Nếu các mệnh đề thuộc TỪ HAI Dạng vật lý khác nhau trở lên -> trả về "${globalTongHop}" (isNew=false, vì đây là nhãn tổng hợp có sẵn).
   - Chỉ tự đề xuất Dạng vật lý mới cho câu Đúng/Sai khi cả 4 mệnh đề cùng một dạng NHƯNG dạng đó chưa có trong danh sách.
3. Nếu Dạng vật lý trả về TRÙNG (không phân biệt hoa/thường) với một dạng đã có trong danh sách -> isNew=false và viết lại NGUYÊN VĂN đúng như trong danh sách.
...
- "Vận dụng cao": bài tập phức tạp, nhiều tầng lập luận, toán thực tế khó, hoặc cần ý tưởng đặc biệt.
