...
//
// Việc khớp tên dạng vật lý về đúng danh mục trong kho KHÔNG làm ở đây - hàm này chỉ
...
function taoPrompt(danhSachDang: string[]): string {
  const keDang = danhSachDang.length
    ? `\n\nDANH SÁCH DẠNG VẬT LÝ ĐANG CÓ TRONG NGÂN HÀNG (hãy chép CHÍNH XÁC tên từ danh sách này nếu tìm được dòng tương ứng):\n- ${danhSachDang.join("\n- ")}`
    : "";
...
[
  {
    ...
    "dangToan": "tên dạng vật lý / nội dung đơn vị kiến thức",
...
export interface NguonKho {
  ...
  danhSachBai: string[];
  /** Tên các dạng vật lý kho đang có. */
  danhSachDang: string[];
  /** Các dạng vật lý thuộc một bài. */
...
export interface KetQuaKhopDong {
  ...
  bai: string | null;
  /** Dạng vật lý chốt lại để đưa vào ma trận. */
  dang: string | null;
  diem: number;
  nguon: NguonKhop;
}
...
/**
 * Khớp một dòng ma trận máy đọc được về dạng vật lý trong kho.
 *
 * Thứ tự thử là BÀI TRƯỚC, DẠNG SAU - không phải ngược lại. Bảng ma trận theo Công
 * văn 7991 có cột "Nội dung / đơn vị kiến thức" ghi TÊN BÀI HỌC, không ghi dạng vật lý.
