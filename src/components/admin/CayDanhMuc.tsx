...
export default function CayDanhMuc({ danhMuc, onSua, onXemCau }: {
  ...
  const O = ({ n, nhat }: { n: number; nhat?: boolean }) => (
    <span className={`inline-block min-w-[26px] text-center px-1 rounded text-[11px] font-bold ${
      n === 0 ? 'bg-rose-50 text-rose-400' : nhat ? 'bg-slate-100 text-slate-600' : 'bg-amber-50 text-amber-700'}`}>{n}</span>
  );
        ...
        <div className="p-3 border-b border-gray-100">
          <div className="relative">
            ...
            <input value={tim} onChange={e => setTim(e.target.value)} placeholder="Tìm chương, bài, dạng…"
                   className="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-500" />
          </div>
            ...
            return (
              <div key={g} className="mb-1">
                <button onClick={() => setMoLop(m => ({ ...m, [g]: !mo }))}
                  ...
                  <span className="w-7 h-7 rounded-lg bg-amber-50 text-amber-700 font-black text-xs flex items-center justify-center">{g}</span>
                      ...
                      return (
                        <button key={c} onClick={() => setChuongChon(k)}
                                className={`w-full text-left px-2.5 py-1.5 rounded-lg flex items-center gap-2 text-[13px] transition-colors ${
                                  chon ? 'bg-amber-600 text-white font-bold' : 'text-gray-700 hover:bg-amber-50'}`}>
                          <span className="flex-1 min-w-0 truncate">{c}</span>
                          <span className={`shrink-0 text-[11px] font-bold ${chon ? 'text-amber-100' : 'text-gray-400'}`}>
                            {soDang} / {dem ? d.tong : '…'}
                          </span>
                        </button>
                      );
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {cacDang.map(d => {
                          ...
                          return (
                            <tr key={d.id} className={`group ${dd.tong === 0 ? 'bg-rose-50/40' : 'hover:bg-amber-50/30'}`}>
