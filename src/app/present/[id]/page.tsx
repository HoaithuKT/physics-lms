        ...
        <div className="w-full flex flex-col" style={{ fontSize: `${fontSize}px` }}>
            ...
            <div className="font-semibold text-slate-800 mb-[1.5em] drop-shadow-sm">
                <div className="prose prose-slate max-w-none [&_.katex]:text-[1.1em] [&_.katex-display]:my-[0.5em]">
                    <ReactMarkdown remarkPlugins={[remarkPhysics, remarkBreaks]} rehypePlugins={[rehypeKatex, rehypeRaw]}>
                        {quizData.question || ""}
                    </ReactMarkdown>
                </div>
                {quizData.img_url && (
                        ...
                        return (
                            ...
                            >
                                <div className="flex-1 min-w-0 text-center text-[1.2em] font-black uppercase text-gray-700">
                                   <ReactMarkdown components={customMarkdownComponents} remarkPlugins={[remarkPhysics, remarkBreaks]} rehypePlugins={[rehypeKatex, rehypeRaw]}>{String(text).replace(/^(\s*\d+)\.(?=\s|$)/, '$1\\.')}</ReactMarkdown>
                                </div>
                            </button>
                        );
                    })}
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="font-medium flex-1 prose prose-slate max-w-none [&_.katex]:text-[1.1em]" style={{ fontSize: '0.9em' }}>
                                    <ReactMarkdown remarkPlugins={[remarkPhysics, remarkBreaks]} rehypePlugins={[rehypeKatex, rehypeRaw]}>{String(opt).replace(/^(\s*\d+)\.(?=\s|$)/, '$1\\.')}</ReactMarkdown>
                                </div>
                            </button>
                        );
                    })}
