  ...
  return (
    <div className="my-6 bg-white border-2 border-indigo-100 rounded-2xl p-5 md:p-8 shadow-sm">
      <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-start gap-2">
         ...
         <ReactMarkdown components={customMarkdownComponents} remarkPlugins={[remarkPhysics]} rehypePlugins={[rehypeKatex]}>{data.question}</ReactMarkdown>
      </h3>
             ...
             return (
               ...
               >
                  ...
                  <div className="prose prose-sm max-w-none text-gray-700">
                     <ReactMarkdown components={customMarkdownComponents} remarkPlugins={[remarkPhysics]} rehypePlugins={[rehypeKatex]}>{opt}</ReactMarkdown>
                  </div>
               </button>
             );
          })}
      ...
      {type === 'true_false_cluster' && (
         <div className="flex flex-col gap-4">
            <div className="text-sm font-semibold text-orange-700 bg-orange-50 px-3 py-2 rounded-lg mb-2">
               ...
               return (
                  <div key={idx} className={`flex flex-col md:flex-row md:items-center justify-between p-4 rounded-xl border-2 transition-all gap-4 ${st}`}>
                     <div className="flex items-start gap-3">
                        ...
                        <div className="prose prose-sm max-w-none text-gray-700">
                           <ReactMarkdown components={customMarkdownComponents} remarkPlugins={[remarkPhysics]} rehypePlugins={[rehypeKatex]}>{opt.content}</ReactMarkdown>
                        </div>
                     </div>
      ...
      {parts.map((p, idx) => {
         if (p.type === 'md') {
             return (
               ...
               ">
                 <ReactMarkdown components={customMarkdownComponents} remarkPlugins={[remarkPhysics]} rehypePlugins={[rehypeKatex]}>{p.content}</ReactMarkdown>
               </div>
             );
         } else if (p.type === 'quiz') {
