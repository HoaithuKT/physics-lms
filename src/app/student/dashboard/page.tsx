...
const MUC_HOC_SINH: MucHocSinh[] = [
  ...
  { tab: 'courses',  nhan: 'Khóa học',   Icon: BookOpen,      mau: 'bg-orange-50 text-orange-600' },
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
        <div className="bg-white p-12 rounded-[2rem] shadow-xl border border-gray-100 max-w-lg text-center relative overflow-hidden">
          ...
          <p className="text-gray-500 leading-relaxed font-medium">
            Hồ sơ học sinh <strong className="text-orange-700">{profile?.full_name}</strong> đã được ghi nhận vào hệ thống. Tuy nhiên, bạn cần đợi Giáo viên hoặc Quản trị viên kích hoạt tài khoản thì mới có thể vào lớp học.
          </p>
          ...
          <button onClick={handleLogout} className="mt-8 text-orange-600 font-bold hover:text-orange-700 flex items-center justify-center gap-2 mx-auto transition-colors">
      ...
      <div className="p-3 sm:p-8 max-w-[1400px] mx-auto space-y-4 sm:space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
          <div>
            ...
            <p className="text-[13px] md:text-base text-gray-500 md:mt-1.5 font-medium">
              Chào <span className="font-bold text-orange-600">{profile?.full_name}</span>
        ...
        <div className="hidden md:flex bg-white p-2.5 rounded-2xl border border-gray-100 shadow-sm flex-row overflow-x-auto no-scrollbar gap-2 items-stretch justify-start md:justify-between relative z-10">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'schedule' ? 'bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'info' ? 'bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'courses' ? 'bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'exams' ? 'bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl font-bold transition-all text-sm sm:text-base whitespace-nowrap ${activeTab === 'finance' ? 'bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-800'}`}
          >
          ...
          >
            <Library className="w-5 h-5 text-indigo-500" /> Sổ tay Vật lý
          </Link>
        </div>
        ...
        <div className="md:hidden grid grid-cols-3 gap-2.5">
          {MUC_HOC_SINH.map(m => {
            ...
            const noiDung = (
              <>
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-1 transition-colors ${dangChon ? 'bg-gradient-to-br from-orange-500 to-amber-600 text-white shadow-md shadow-orange-500/25' : m.mau}`}>
                  <m.Icon className="w-[18px] h-[18px]" />
                </div>
                <span className={`text-[11px] font-bold leading-tight text-center ${dangChon ? 'text-orange-700' : 'text-gray-600'}`}>{m.nhan}</span>
              </>
            );
        ...
        <div className="w-full">
          ...
          {activeTab === 'schedule' && (
            <div className="w-full relative group animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {!profile?.course_id ? (
                ...
                <div className="rounded-[2.5rem] overflow-hidden shadow-2xl shadow-orange-900/10 border-[8px] border-white bg-white w-full max-w-6xl mx-auto flex justify-center">
          ...
          {activeTab === 'info' && (
            <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-orange-600 relative p-5 sm:p-14 text-white flex items-center gap-4 sm:gap-8 overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-br from-orange-600/90 to-amber-800/90 mix-blend-multiply"></div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="inline-flex items-center gap-2 bg-black/20 backdrop-blur-sm px-4 py-1.5 rounded-full border border-white/10">
                    ...
                    <span className="font-semibold text-orange-50">Hồ sơ Học sinh</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-10 sm:p-14">
                <h3 className="text-base sm:text-xl font-black text-gray-800 mb-4 sm:mb-8 flex items-center gap-2.5 border-b border-gray-100 pb-3 sm:pb-4">
                  <User className="text-orange-600 w-6 h-6"/> Thông tin cá nhân (Chỉ xem)
                </h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8">
                  <div className="group">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Tài khoản đăng nhập</p>
                    <p className="font-bold text-gray-800 text-lg">{profile?.username}</p>
                  </div>
                  <div className="group">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Số điện thoại HS</p>
                    <p className="font-bold text-gray-800 text-lg">{profile?.student_phone || '—'}</p>
                  </div>
                  <div className="group">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Trường</p>
                    <p className="font-bold text-gray-800 text-lg">{profile?.school || '—'}</p>
                  </div>
                  <div className="group">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Lớp</p>
                    <p className="font-bold text-gray-800 text-lg">{profile?.class_name || '—'}</p>
                  </div>
                  <div className="group border-t border-gray-50 pt-6 mt-2">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Họ tên Phụ huynh</p>
                    <p className="font-bold text-gray-800 text-lg">{profile?.parent_name || '—'}</p>
                  </div>
                  <div className="group border-t border-gray-50 pt-6 mt-2">
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1 group-hover:text-orange-500 transition-colors">Số ĐT Phụ huynh</p>
                ...
                <div className="mt-12 pt-10 border-t border-gray-100">
                  {!showPasswordForm ? (
                    <button onClick={() => setShowPasswordForm(true)} className="flex items-center gap-2 font-bold text-orange-600 bg-orange-50 border border-orange-100 px-6 py-3 rounded-xl hover:bg-orange-100 hover:shadow-sm transition-all">
                    ...
                    <form onSubmit={handleChangePassword} className="max-w-md bg-white border border-gray-200 shadow-xl shadow-gray-200/50 p-8 rounded-2xl space-y-5 animate-in fade-in zoom-in-95">
                      ...
                      <div>
                        <div className="relative">
                          <input type={showNewPw ? "text" : "password"} required value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Mật khẩu mới (ít nhất 6 ký tự)" className="w-full px-5 py-3.5 pr-12 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 font-medium transition-all" />
                      ...
                      <div>
                        <input type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="Xác nhận lại mật khẩu mới" className="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 font-medium transition-all" />
                      </div>
                      ...
                      <div className="flex gap-3 pt-4">
                        <button type="submit" disabled={pwLoading} className="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-bold py-3.5 rounded-xl disabled:opacity-50 shadow-lg shadow-orange-600/30 transition-all">
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-6xl mx-auto">
              ...
              {courses.length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-8">
                  {courses.map((course) => (
                    <div key={course.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm hover:shadow-2xl hover:shadow-orange-900/5 transition-all duration-300 hover:-translate-y-2 overflow-hidden group flex flex-col">
                      <div className="h-3 bg-gradient-to-r from-orange-500 to-amber-500 w-full"></div>
                      <div className="p-4 sm:p-10 flex flex-col flex-1">
                        <div className="flex justify-between items-start mb-6">
                          <span className="bg-orange-50 text-orange-700 px-4 py-1.5 rounded-full text-sm font-black border border-orange-100 tracking-wide uppercase">
                            Lớp {course.grade_level}
                          </span>
                        </div>
                        <h3 className="text-base sm:text-2xl font-black text-gray-800 mb-2 sm:mb-3 group-hover:text-orange-600 transition-colors line-clamp-2 leading-tight">
                          {course.title}
                        </h3>
                        ...
                        <div className="bg-gray-50 rounded-2xl p-5 flex items-center justify-between mt-auto group-hover:bg-orange-50 transition-colors border border-gray-100">
                          ...
                          <button onClick={() => setExpandedCourseId(expandedCourseId === course.id ? null : course.id)} className="flex items-center gap-2 bg-orange-600 text-white px-6 py-3 rounded-xl font-bold shadow-md shadow-orange-600/20 hover:bg-orange-700 transition-all">
                                 ...
                                 {course.chapters.map((chapter: any) => {
                                    ...
                                    return (
                                      <div key={chapter.id} className="space-y-2">
                                        <h5 className="font-bold text-orange-700 text-sm uppercase tracking-wide">{chapter.title}</h5>
                                        {chapterLessons.length > 0 ? (
                                          <div className="flex flex-col gap-1 pl-3 border-l-2 border-orange-100">
                                            {chapterLessons.map((lesson: any) => (
                                              <Link key={lesson.id} href={`/student/lessons/${lesson.id}`} className="block px-3 py-2 hover:bg-orange-50 rounded-lg text-gray-600 hover:text-orange-700 transition-colors text-sm font-medium">
                                                {lesson.title}
                                              </Link>
                                            ))}
                                          </div>
                        ...
                        {tuitionRecords.map(t => {
                          ...
                          return (
                            <tr key={t.id} className="hover:bg-gray-50/50">
                              ...
                              <td className="px-5 py-4 text-right font-bold text-orange-700">{total.toLocaleString('vi-VN')} đ</td>
          ...
          {MUC_HOC_SINH.filter(m => MUC_THANH_DUOI.includes(m.tab)).map(m => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-0.5 py-2 transition-colors ${dangChon ? 'text-orange-600' : 'text-gray-400'}`}
              >
