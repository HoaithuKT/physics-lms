    ...
    <div className="relative bg-white p-8 md:p-10 rounded-[2rem] border-4 border-indigo-50 my-12 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] not-prose animate-in zoom-in-95 duration-500 origin-center overflow-hidden">
      ...
      <div className="flex flex-col gap-4">
        {data.options.map((opt: string, i: number) => {
          ...
          if (selected === i) {
            if (isCorrect) {
              btnClass = "bg-amber-500 text-white border-amber-600 shadow-[0_10px_25px_-5px_rgba(16,185,129,0.5)] z-10";
              iconClass = "bg-amber-600 text-white shadow-inner";
            ...
            btnClass = "bg-amber-50 text-amber-800 border-amber-200"; 
      ...
      {isCorrect === true && (
        <div className="mt-8 p-6 bg-amber-100 text-amber-800 rounded-2xl font-bold text-xl flex items-center justify-center gap-3 animate-in slide-in-from-bottom-6 zoom-in-95 duration-500 border border-amber-300 shadow-[0_10px_30px_-10px_rgba(16,185,129,0.3)]">
...
export default function CoursePlayerPage() {
  ...
  const renderMarkdownComponent = {
    ...
    h2: ({ children }: any) => <h2 className="text-2xl font-bold text-orange-700 mt-8 mb-4 flex items-center gap-2"><div className="w-2 h-6 bg-orange-500 rounded-full"></div>{children}</h2>,
  ...
  return (
    <div className="h-screen flex flex-col bg-[#fff7ed] font-sans overflow-hidden">
      {/* Top Navbar */}
      <header className="bg-white h-16 border-b border-gray-200 flex items-center justify-between px-4 z-20 shrink-0">
        <div className="flex items-center gap-4">
          <div className="hidden md:block bg-orange-100 p-2 rounded-lg text-orange-600">
            <BookOpen className="w-5 h-5" />
          </div>
          <div>
            <div className="text-xs font-bold text-orange-600 uppercase tracking-wider mb-0.5">{course?.title}</div>
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="px-4 py-2 bg-orange-50 hover:bg-orange-100 text-orange-700 font-bold rounded-lg transition-colors flex items-center gap-2"
          >
            ...
            {activeModule?.content_markdown && (
              <div ref={contentRef} className={`bg-white shadow-lg border border-gray-100 relative transition-all duration-300 flex flex-col ${isContentFullscreen ? 'h-screen w-screen overflow-y-auto rounded-none p-8 md:p-16' : 'rounded-2xl p-6 md:p-10 min-h-[60vh]'}`}>
                <div className="absolute top-4 right-4 z-10">
                  <button onClick={toggleContentFullscreen} className={`p-2.5 rounded-lg transition-all shadow-sm border ${isContentFullscreen ? 'bg-gray-100 hover:bg-gray-200 text-gray-700 border-gray-300' : 'bg-white hover:bg-gray-50 text-gray-500 border-gray-200 hover:text-orange-600 hover:border-orange-200'}`}>
                    ...
                    return (
                      <>
                        <div className="w-full bg-gray-200 h-2 rounded-full mb-8 overflow-hidden flex-shrink-0">
                          <div className="bg-orange-500 h-full transition-all duration-500 ease-out" style={{ width: `${((currentSlideIndex + 1) / totalSlides) * 100}%` }}></div>
                        </div>
                        ...
                        <div className="flex justify-between items-center mt-auto pt-6 border-t border-gray-100 flex-shrink-0 w-full">
                          <button disabled={currentSlideIndex === 0} onClick={() => { setCurrentSlideIndex(prev => prev - 1); window.scrollTo({ top: contentRef.current?.offsetTop, behavior: 'smooth' }); }} className="flex items-center gap-1.5 px-5 py-2.5 rounded-full font-bold text-orange-700 bg-orange-50 hover:bg-orange-100 disabled:opacity-40 disabled:hover:bg-orange-50 transition-colors shadow-sm"><ChevronLeft className="w-5 h-5" /> Quay Lại</button>
                          ...
                          <button disabled={currentSlideIndex >= totalSlides - 1 || !canGoNext} onClick={() => { setCurrentSlideIndex(prev => prev + 1); window.scrollTo({ top: contentRef.current?.offsetTop, behavior: 'smooth' }); }} className={`flex items-center gap-1.5 px-6 py-2.5 rounded-full font-bold transition-colors shadow-sm ${canGoNext ? 'text-white bg-orange-600 hover:bg-orange-700' : 'text-gray-400 bg-gray-200 cursor-not-allowed'}`} title={!canGoNext ? "Bạn phải trả lời đúng Trắc nghiệm để đi tiếp!" : ""}>
                        ...
                        <div key={lesson.id} className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
                          ...
                          <div className="py-1">
                            {lessonModules.map(mod => {
                              ...
                              if (mod.type === 'document') icon = <Download className="w-4 h-4 text-orange-500" />;
                              ...
                              return (
                                <button 
                                  ...
                                  className={`w-full text-left px-4 py-2.5 flex items-center gap-3 transition-colors ${
                                    isActiveMod ? 'bg-orange-50 border-l-4 border-orange-500' : 'hover:bg-gray-50 border-l-4 border-transparent'
                                  }`}
                                >
                                  {icon}
                                  <span className={`text-sm ${isActiveMod ? 'font-bold text-orange-700' : 'font-medium text-gray-600'}`}>
                                    {mod.title}
                                  </span>
                                </button>
                              );
