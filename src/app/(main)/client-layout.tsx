  ...
  return (
    <div className="flex h-screen bg-[#fff7ed] font-sans">
      ...
      <aside 
        ...
        } bg-[#f97316] text-white flex flex-col transition-all duration-300 relative`}
      >
        {/* Toggle Button */}
        <button 
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="absolute -right-3 top-6 bg-white text-[#f97316] p-1.5 rounded-full shadow-md border border-gray-100 hover:bg-gray-50 z-10"
        >
        ...
        <div className={`p-6 flex items-center gap-3 ${isCollapsed ? 'justify-center px-0' : ''}`}>
          <div className="bg-white text-[#f97316] p-2 rounded-xl flex-shrink-0">
            <BookOpen className="w-6 h-6" />
          </div>
          {!isCollapsed && (
            <div className="overflow-hidden">
              <h1 className="font-bold text-xl whitespace-nowrap">Physics LMS</h1>
              <p className="text-xs text-orange-100 whitespace-nowrap">HỆ THỐNG QUẢN LÝ</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto mt-4 pb-10 overflow-x-hidden no-scrollbar">
          ...
          <div className="px-4 mb-6">
            {!isCollapsed && <p className="text-xs font-bold text-orange-600/50 uppercase tracking-wider mb-4 px-2">Quản lý đào tạo</p>}
                    ...
                    <Link 
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors ${
                        isActive 
                          ...
                          : "text-orange-50 hover:bg-white/5"
          ...
          {/* Content Nav */}
          <div className="px-4 mb-6">
            {!isCollapsed && <p className="text-xs font-bold text-orange-600/50 uppercase tracking-wider mb-4 px-2">Học liệu & Thi cử</p>}
            <ul className="space-y-1">
              {contentNav.map((item) => {
