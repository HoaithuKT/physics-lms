    ...
    <div className="space-y-6 w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition-all flex items-center justify-center gap-2 hover:shadow-md hover:-translate-y-0.5 active:translate-y-0"
        >
      ...
      <div className="flex items-center gap-2 border-b border-gray-200">
        <button
          ...
          className={`px-6 py-3 font-bold text-sm transition-colors relative ${activeTab === 'GRADE' ? 'text-orange-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        >
          Khối Lớp
          {activeTab === 'GRADE' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-orange-600 rounded-t-full" />}
        </button>
        <button
          onClick={() => setActiveTab('CATEGORY')}
          className={`px-6 py-3 font-bold text-sm transition-colors relative ${activeTab === 'CATEGORY' ? 'text-orange-600' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        >
          Danh Mục Khóa Học
          {activeTab === 'CATEGORY' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-orange-600 rounded-t-full" />}
        </button>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4">
        <div className="relative w-full max-w-md">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:bg-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all text-sm"
          />
        </div>
      </div>
            ...
            <form onSubmit={editingCategory ? handleUpdateCategory : handleCreateCategory} className="p-6 space-y-5">
              <div>
                ...
                <div className="flex items-center gap-4 mb-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="catType" checked={newCategoryType === 'GRADE'} onChange={() => setNewCategoryType('GRADE')} className="w-4 h-4 text-orange-600" />
                    <span className="text-sm font-medium text-gray-700">Khối Lớp</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="catType" checked={newCategoryType === 'CATEGORY'} onChange={() => setNewCategoryType('CATEGORY')} className="w-4 h-4 text-orange-600" />
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
                />
              </div>
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-gray-100">
                ...
                <button
                  ...
                  className="px-5 py-2.5 text-sm font-bold text-white bg-orange-600 hover:bg-orange-700 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                >
