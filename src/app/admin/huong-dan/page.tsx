...
function TrangHuongDan() {
  ...
  return (
    <div className="max-w-[1180px] mx-auto">
      {/* Dải đầu trang */}
      <div id="cam-nang-dau" className="scroll-mt-[88px] rounded-3xl bg-gradient-to-r from-orange-600 to-orange-500 text-white px-6 py-5 md:px-8 md:py-6 mb-5 shadow-sm">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[13px] text-orange-50/90 mt-0.5">{bai.mota}</p>
          </div>
            ...
            return (
              <button key={ma} onClick={() => doiBai(ma)}
                      className={`flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-[13px] font-bold transition-colors ${
                        dang ? 'bg-white text-orange-700 shadow-sm' : 'bg-white/15 text-white hover:bg-white/25'}`}>
            ...
            {muc.map((m, i) => (
              <button key={i} onClick={() => nhay(i)}
                ...
                <span className={`w-5 h-5 rounded-md text-[11px] font-black flex items-center justify-center shrink-0 ${
                  mucDangXem === i ? 'bg-orange-600 text-white' : 'bg-slate-100 text-slate-500'}`}>
                  {i + 1}
                </span>
              ...
              {muc.map((m, i) => (
                <button key={i} onClick={() => nhay(i)}
                        className="px-2.5 py-1 rounded-full text-[11.5px] font-bold bg-slate-50 border border-slate-200
                                   text-slate-600 hover:border-orange-400 hover:text-orange-700 transition-colors">
                  {i + 1}. {boSo(m.ten)}
                </button>
              ))}
            </div>
            ...
            return (
              <section key={`${maBai}-${i}`} id={`cam-nang-muc-${i}`} className="mb-9 scroll-mt-[88px]">
                <h2 className="flex items-center gap-2.5 mb-3 pb-2 border-b-2 border-slate-100">
                  <span className="w-7 h-7 rounded-lg bg-orange-600 text-white text-[13px]
