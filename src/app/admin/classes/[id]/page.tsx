...
export default function ClassDetailsPage() {
  ...
  if (loading) return <div className="p-8 flex justify-center"><Loader2 className="w-10 h-10 animate-spin text-orange-600" /></div>;
  if (!classInfo) return <div className="p-8 text-center text-red-500">Không tìm thấy Lớp học!</div>;

  return (
    <div className="p-8 w-full max-w-7xl mx-auto">
      <Link href="/admin/classes" className="inline-flex items-center gap-2 text-gray-500 hover:text-orange-600 mb-6 font-medium transition-colors">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-orange-400 to-indigo-500" />
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            <span className="text-sm font-bold uppercase tracking-wider text-orange-600 bg-orange-50 px-3 py-1 rounded-full mb-3 inline-block">
      ...
      <div className="flex gap-2 overflow-x-auto custom-scrollbar mb-6 pb-2 border-b border-gray-100">
        <button 
          ...
          className={`flex items-center gap-2 px-5 py-3 font-bold rounded-t-xl transition-all border-b-2 ${activeTab === 'students' ? 'border-orange-600 text-orange-700 bg-orange-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        >
          <Users size={18} /> Danh sách Học sinh
        </button>
        <button 
          onClick={() => setActiveTab('attendance')} 
          className={`flex items-center gap-2 px-5 py-3 font-bold rounded-t-xl transition-all border-b-2 ${activeTab === 'attendance' ? 'border-orange-600 text-orange-700 bg-orange-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        >
          <CheckSquare size={18} /> Điểm danh
        </button>
        <button 
          onClick={() => setActiveTab('tuition')} 
          className={`flex items-center gap-2 px-5 py-3 font-bold rounded-t-xl transition-all border-b-2 ${activeTab === 'tuition' ? 'border-orange-600 text-orange-700 bg-orange-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}
        >
      ...
      {activeTab === 'students' && (
        ...
        <div className="flex gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-orange-600 text-white px-4 py-2.5 rounded-xl font-bold hover:bg-orange-700 transition-colors"
          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[80vh]">
            ...
            <div className="flex border-b border-gray-100">
              <button 
                ...
                className={`flex-1 py-3 text-sm font-bold transition-colors ${modalTab === 'search' ? 'border-b-2 border-orange-600 text-orange-700' : 'text-gray-500 hover:bg-gray-50'}`}
              >
                Tìm học sinh có sẵn
              </button>
              <button 
                onClick={() => setModalTab('create')} 
                className={`flex-1 py-3 text-sm font-bold transition-colors ${modalTab === 'create' ? 'border-b-2 border-orange-600 text-orange-700' : 'text-gray-500 hover:bg-gray-50'}`}
              >
                Tạo học sinh mới
              </button>
            </div>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:bg-white focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500"
                      />
                    </div>
                    <button 
                      onClick={handleSearchStudents} disabled={searching}
                      className="px-6 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-colors"
                    >
                  ...
                  {searchResults.length > 0 ? (
                    <div className="space-y-3">
                      {searchResults.map(hs => (
                        <div key={hs.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:border-orange-200 transition-colors">
                          <div>
                          ...
                          <button 
                            ...
                            className="px-4 py-2 text-sm font-bold bg-orange-50 text-orange-700 hover:bg-orange-600 hover:text-white rounded-lg transition-colors flex items-center gap-2"
                          >
              ...
              <form onSubmit={handleCreateStudent} className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" required value={createForm.full_name} onChange={e => setCreateForm({...createForm, full_name: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="Nguyễn Văn A" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Tài khoản *</label>
                    <input type="text" required value={createForm.username} onChange={e => setCreateForm({...createForm, username: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="nguyenvana123" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Mật khẩu</label>
                    <input type="text" value={createForm.password} onChange={e => setCreateForm({...createForm, password: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="Mặc định: 123456" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">SĐT Học sinh</label>
                    <input type="text" value={createForm.student_phone} onChange={e => setCreateForm({...createForm, student_phone: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="09xxxx..." />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Họ tên Phụ huynh</label>
                    <input type="text" value={createForm.parent_name} onChange={e => setCreateForm({...createForm, parent_name: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="Phụ huynh..." />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">SĐT Phụ huynh</label>
                    <input type="text" value={createForm.parent_phone} onChange={e => setCreateForm({...createForm, parent_phone: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="09xxxx..." />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Trường</label>
                    <input type="text" value={createForm.school} onChange={e => setCreateForm({...createForm, school: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="THPT..." />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Lớp</label>
                    <input type="text" value={createForm.class_name} onChange={e => setCreateForm({...createForm, class_name: e.target.value})} className="w-full p-2 border rounded-lg outline-none focus:border-orange-500 bg-white" placeholder="12A1" />
                  </div>
                </div>
                <div className="pt-4 flex justify-end">
                  <button type="submit" disabled={isCreating} className="px-6 py-2.5 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 transition-colors flex items-center gap-2">
              ...
              {!importResults ? (
                <form onSubmit={handleImportExcel} className="space-y-6">
                  ...
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center hover:bg-gray-50 transition-colors cursor-pointer relative group">
                    ...
                    />
                    <FileSpreadsheet className="w-12 h-12 text-orange-500 mx-auto mb-3 group-hover:scale-110 transition-transform" />
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" disabled={!importFile || importing} className="px-6 py-2.5 rounded-xl bg-orange-600 text-white font-bold hover:bg-orange-700 shadow-sm transition-all flex items-center gap-2 disabled:opacity-50">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editForm.student_phone} onChange={e => setEditForm({...editForm, student_phone: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Trường học</label>
                  <input type="text" value={editForm.school} onChange={e => setEditForm({...editForm, school: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Họ tên Phụ huynh</label>
                  <input type="text" value={editForm.parent_name} onChange={e => setEditForm({...editForm, parent_name: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">SĐT Phụ huynh</label>
                  <input type="text" value={editForm.parent_phone} onChange={e => setEditForm({...editForm, parent_phone: e.target.value})} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:bg-white transition-colors" />
                </div>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50">
              ...
              <button onClick={handleSaveEdit} disabled={savingEdit} className="px-5 py-2.5 bg-orange-600 text-white font-bold rounded-xl hover:bg-orange-700 transition-all flex items-center gap-2 disabled:opacity-50">
