...
export default function AttendanceTab({ classId, enrollments, className }: { classId: string, enrollments: any[], className?: string }) {
  ...
  if (loading) return <div className="py-10 text-center"><Loader2 className="w-8 h-8 animate-spin text-orange-600 mx-auto" /></div>;
          ...
          <div className="max-h-[600px] overflow-y-auto">
            ...
            {!hasTodaySession && (
               <div 
                  ...
                  className={`p-4 border-b border-gray-50 cursor-pointer transition-colors flex justify-between items-center ${selectedSessionId === "NEW_TODAY" ? 'bg-orange-50 border-l-4 border-l-orange-500' : 'hover:bg-gray-50'}`}
                >
                  <div>
                    <div className={`font-bold ${selectedSessionId === "NEW_TODAY" ? 'text-orange-700' : 'text-gray-800'}`}>Hôm nay</div>
            ...
            {sessions.map(session => (
              <div 
                ...
                className={`p-4 border-b border-gray-50 cursor-pointer transition-colors flex justify-between items-center group ${selectedSessionId === session.id ? 'bg-orange-50 border-l-4 border-l-orange-500' : 'hover:bg-gray-50'}`}
              >
                <div>
                  <div className={`font-bold ${selectedSessionId === session.id ? 'text-orange-700' : 'text-gray-800'}`}>{session.title}</div>
          ...
          <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={handleSave} 
                disabled={saving}
                className="bg-orange-600 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-orange-700 transition-colors disabled:opacity-50 shadow-sm shadow-orange-600/20"
              >
                      ...
                      <tr key={stId} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <input 
                            ...
                            className="w-full text-sm px-3 py-1.5 border border-gray-200 rounded-lg focus:outline-none focus:border-orange-500"
                          />
                        </td>
                      </tr>
                    );
        ...
        <div ref={printRef} className="w-[850px] bg-white p-0 font-sans border-0 relative">
          <div className="bg-rose-400 rounded-[2rem] p-3">
             <div className="bg-rose-50 rounded-[1.5rem] p-8 border-4 border-white flex flex-col h-full relative overflow-hidden">
                ...
                {/* Student List */}
                <div className="bg-white rounded-2xl shadow-sm border border-amber-100 overflow-hidden relative z-10 mb-6">
                  <table className="w-full text-left">
                    <thead>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {enrollments.filter(en => ['LATE', 'EXCUSED_ABSENCE', 'UNEXCUSED_ABSENCE'].includes(attendance[en.profiles.id]?.status)).length === 0 ? (
                        <tr>
                          <td colSpan={3} className="py-10 text-center text-amber-600 font-bold text-lg bg-amber-50/30">
                        ...
                        enrollments.filter(en => ['LATE', 'EXCUSED_ABSENCE', 'UNEXCUSED_ABSENCE'].includes(attendance[en.profiles.id]?.status)).map((en, index) => {
                          ...
                          return (
                            <tr key={en.profiles.id} className="hover:bg-gray-50/50 transition-colors">
                              <td className="py-2 px-3 text-center font-bold text-gray-500">{index + 1}</td>
