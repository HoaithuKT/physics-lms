  ...
  ({ results, classNameName, lessonName }, ref) => {
    ...
    return (
      <div 
        ...
        className="p-8 rounded-3xl overflow-hidden relative font-sans text-zinc-900 border-8 border-orange-600/20 shadow-2xl"
      >
        {/* Background Decorations */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-orange-200 rounded-full blur-3xl opacity-50 -translate-y-1/2 translate-x-1/4" />
        <div className="absolute bottom-0 left-0 w-80 h-80 bg-amber-200 rounded-full blur-3xl opacity-40 translate-y-1/3 -translate-x-1/4" />
        
        {/* Header */}
        <div className="relative z-10 text-center mb-8 flex flex-col items-center">
          <div className="w-20 h-20 bg-orange-600 rounded-full flex items-center justify-center shadow-lg mb-4 text-white">
            <Trophy className="w-10 h-10" />
          </div>
          <h1 className="text-3xl font-black text-orange-900 uppercase tracking-wider mb-2">{title}</h1>
          <h2 className="text-xl font-bold text-orange-700 bg-orange-100/80 px-6 py-2 rounded-full inline-block shadow-sm">
            {subtitle}
          </h2>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-3">
          {validResults.map((r, index) => {
            ...
            if (index === 0) {
              ...
              rankIcon = <Star className="w-5 h-5 text-amber-500" />;
            }
              ...
              <div key={r.id} className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${rankColor}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="bg-white/80 px-4 py-2 rounded-xl shadow-inner border border-zinc-100">
                    <span className="font-black text-2xl text-orange-700">{r.score}</span>
                    <span className="text-sm font-bold text-orange-900/50 ml-1">điểm</span>
                  </div>
                </div>
              </div>
            );
          ...
          {validResults.length === 0 && (
            <div className="text-center p-8 bg-white/50 rounded-2xl border border-dashed border-orange-300 text-orange-800 font-bold">
        ...
        {/* Footer */}
        <div className="relative z-10 mt-8 text-center text-orange-800/60 font-semibold text-sm border-t border-orange-600/10 pt-4">
