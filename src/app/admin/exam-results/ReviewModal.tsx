...
Hãy đưa ra Nhận xét chi tiết cho Học sinh và đánh giá số điểm đạt được (Tối đa ${detail.maxScore} điểm).`;
  ...
  const copyToClipboard = (text: string) => {
    ...
    alert("Đã sao chép Lệnh chấm bài chuẩn! Cô có thể mở tab mới, vào trang web Gemini và dán vào đó (kèm theo thao tác Sao chép hình ảnh).");
  };
...
--- DANH SÁCH CÁC CÂU HỎI TỰ LUẬN ---\n\n`;
  ...
  const copyAllImages = async () => {
    ...
    try {
      ...
      // 2. Tính vật lý kích thước (Giới hạn chiều rộng để khỏi lỗi)
      ...
      await new Promise<void>((resolve, reject) => {
        canvas.toBlob(async (blob) => {
          ...
          try {
            ...
            alert(`Đã gộp thành công ${imagesToCopy.length} ảnh thành 1 ẢNH DUY NHẤT và copy vào bộ nhớ tạm! Cô hãy sang Gemini bấm Ctrl+V để dán.`);
            resolve();
          } catch (err) {
            reject(err);
          }
      ...
      console.error(e);
      alert("Lỗi sao chép: " + e.message + "\nTrình duyệt của Cô có thể chặn tính năng này, vui lòng sao chép thủ công từng ảnh.");
    } finally {
              ...
              <div className="mb-6 p-5 bg-gradient-to-r from-indigo-50 to-blue-50 border-2 border-indigo-200 rounded-2xl shadow-sm animate-in fade-in slide-in-from-top-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3 bg-white p-4 rounded-xl border border-indigo-100">
                    ...
                    <p className="text-[11px] text-zinc-500 italic text-center mt-2">Mẹo: Hệ thống sẽ tự động ghép các ảnh Cô chọn thành 1 ảnh dài duy nhất để copy, giúp lách luật giới hạn 10 ảnh của Gemini.</p>
                  </div>
