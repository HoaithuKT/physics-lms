 ...
 * thành "\f"+"rac", "\right" thành "\r"+"ight".
 *
 * Nay dùng chung docJsonCauHoi (utils/vaJson) như các chỗ đọc JSON khác trong app: ở đó
...
export default function SimilarGeneratorPage() {
  ...
  const handleScanBaseAI = async () => {
    ...
    try {
      ...
      const prompt = `Bạn là chuyên gia Vật lý. Hãy bóc tách các câu hỏi trong ảnh và trả về MẢNG JSON.
  ...
  const generateFreePromptText = () => {
      ...
      return `Bạn là chuyên gia Vật lý. Hãy ĐÓNG VAI LÀ NGƯỜI BIÊN SOẠN và TỰ SÁNG TÁC / TỰ SINH RA các câu hỏi Toán học MỚI HOÀN TOÀN dựa trên yêu cầu sau của giáo viên:
                          ...
                          {bq.generatedVariants.map((v, vIdx) => (
                              <div key={v.temp_id} className="border border-indigo-100 rounded-xl p-4 bg-indigo-50/20 relative">
                                  ...
                                  <div className="text-sm font-medium text-gray-800 mt-2 max-h-40 overflow-y-auto">
                                      <PhysicsRenderer htmlContent={v.content} />
                                  </div>
                              </div>
                          ))}
                      </div>
