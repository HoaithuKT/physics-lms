...
//
// Câu có Chương/Bài/Dạng vật lý không khớp danh mục có sẵn KHÔNG tự thêm âm thầm
 ...
 *
 * Bản cũ gom theo mỗi tên, nên câu của ba lớp khác nhau cùng đề xuất một tên dạng thì dồn
 * chung MỘT thẻ; một lần bấm "Gộp" đóng dấu tên đó lên câu của cả ba lớp. Kho Toán đã hỏng
 * đúng lối này: 29 câu Đúng/Sai của Lớp 10, 11, 12 cùng mang tên một dạng của Hình học 12.
 */
...
export default function BatchQueuePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // questionFingerprint.ts lo. Bản cũ tự bỏ khoảng trắng rồi so khớp tuyệt đối
      // nên chỉ cần lệch một dấu, hay ảnh khác địa chỉ, là bỏ sót.
  ...
  // Lời nhắc trong lúc dựng trang PDF thành ảnh.
   ...
   *
   * Bản cũ bày cả kho: ô "Gộp vào mục có sẵn" của một dạng Lớp 10 vẫn liệt kê mọi tên dạng
   * của mọi lớp, không sắp xếp. Chọn nhầm sang tên của lớp khác là chuyện sớm muộn.
   */
    ...
    try {
      ...
      alert(`Đã lưu ${result.insertedCount} câu vào Ngân hàng!`
        ...
        /* Câu thiếu đáp án vẫn lưu được, nhưng học sinh chọn đúng vẫn báo sai mà không có
           dấu hiệu gì. Bản trước bỏ qua cảnh báo này nên kho Toán tích lại 193 câu như vậy. */
