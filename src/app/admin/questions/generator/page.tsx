...
export default function QuestionGeneratorPage() {
  ...
  const [globalPhysicsForm, setGlobalPhysicsForm] = useState("");
  ...
  const generatePrompt = () => {
    return `Bạn là một chuyên gia Vật lý và chuyên gia ra đề thi trắc nghiệm.
...
3. ĐẢM BẢO CHỐNG TRÙNG LẶP: 4 đáp án phải độc lập tuyệt đối về mặt giá trị Vật lý (Không được có 2 đáp án cùng giá trị như 1/2 và 0.5).
...
6. Mọi công thức Vật lý PHẢI được bọc trong dấu $...$ (ví dụ: $\\frac{1}{2}$). 
[CẢNH BÁO LỖI JSON]: Khi xuất JSON, tất cả các dấu gạch chéo ngược (\\) trong công thức Vật lý BẮT BUỘC phải được nhân đôi thành (\\\\) để tránh lỗi "Bad escaped character". Ví dụ: phải viết là $\\\\frac{1}{2}$ thay vì $\\frac{1}{2}$, $\\\\in$ thay vì $\\in$. Tương tự với \\\\notin, \\\\pi, \\\\sqrt. Khác biệt duy nhất là dùng ký tự \\n (1 gạch chéo) để xuống dòng trong phần Lời giải chi tiết. Khúc này CỰC KỲ QUAN TRỌNG, nếu sai app sẽ bị sập.

[PHÂN LOẠI DANH MỤC]:
Hãy tự động phân loại câu hỏi vào Chuyên đề, Bài học và Dạng vật lý phù hợp nhất dựa trên danh sách có sẵn dưới đây:
${!globalTopic ? `DANH SÁCH CHUYÊN ĐỀ:\n${uniqueTopics.map(t => `- ${t}`).join('\n')}\n` : ''}
${!globalLesson ? `DANH SÁCH BÀI HỌC:\n${uniqueLessons.map(l => `- ${l}`).join('\n')}\n` : ''}
${!globalPhysicsForm ? `DANH SÁCH DẠNG TOÁN:\n${uniqueForms.map(f => `- ${f}`).join('\n')}\n` : ''}
...
[
  {
    ...
    "math_form": "${globalPhysicsForm || "Tên dạng vật lý (Lấy từ danh sách hoặc tự suy luận)"}",
    "question_text": "Nội dung câu hỏi (chỉnh sửa lại từ đề bài gốc cho phù hợp với trắc nghiệm, bọc vật lý bằng $...$)",
  ...
  const processAndShuffleOutput = (items: any[]) => {
    const results = items.map((jsonObj, idx) => {
      ...
      return {
        ...
        math_form: globalPhysicsForm || jsonObj.math_form,
  ...
  const normalizePhysicsString = (str: string) => {
    if (!str) return "";
    return str
  ...
  const handleSaveToBank = async () => {
    ...
    try {
      ...
      const existingNormalized = (existingQuestions || []).map(q => normalizePhysicsString(q.content));
      const inserts = [];
      let duplicateCount = 0;

      for (let i = 0; i < generatedResults.length; i++) {
        const res = generatedResults[i];
        const normContent = normalizePhysicsString(res.content);
      ...
      /* Cảnh báo "còn khuyết" phải hiện ra cho Thầy cô đọc, không phải chỉ ghi vào console.
         Đo trên kho Toán: 193 câu đang trống đáp án - trắc nghiệm chọn đúng vẫn báo sai,
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex gap-3 overflow-x-auto">
        <select value={globalGrade} onChange={e => { setGlobalGrade(e.target.value); setGlobalTopic(""); setGlobalLesson(""); setGlobalPhysicsForm(""); }} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold min-w-[100px] outline-none">
          <option value="">-- Lớp --</option>
          {uniqueGrades.map(g => <option key={g} value={g}>Lớp {g}</option>)}
        </select>
        <select value={globalSubject} onChange={e => { setGlobalSubject(e.target.value); setGlobalTopic(""); setGlobalLesson(""); setGlobalPhysicsForm(""); }} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold min-w-[120px] outline-none">
          <option value="">-- Phân môn --</option>
          {uniqueSubjects.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={globalTopic} onChange={e => { setGlobalTopic(e.target.value); setGlobalLesson(""); setGlobalPhysicsForm(""); }} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold flex-1 min-w-[200px] outline-none">
          <option value="">-- Chọn Chương / Chuyên đề --</option>
          {uniqueTopics.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <select value={globalLesson} onChange={e => { setGlobalLesson(e.target.value); setGlobalPhysicsForm(""); }} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold flex-1 min-w-[200px] outline-none">
          <option value="">-- Chọn Bài học --</option>
          {uniqueLessons.map(l => <option key={l} value={l}>{l}</option>)}
        </select>
        <select value={globalPhysicsForm} onChange={e => setGlobalPhysicsForm(e.target.value)} className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold flex-1 min-w-[200px] outline-none">
          <option value="">-- Chọn Dạng vật lý --</option>
                            ...
                            <div key={res.id} className="bg-indigo-50/30 p-6 rounded-3xl border-2 border-indigo-100 relative group">
                                ...
                                {res.isPreviewMode !== false ? (
                                    <div className="w-full min-h-[100px] p-4 bg-white border border-indigo-200 rounded-xl mb-4 font-medium text-slate-800 prose prose-slate max-w-none">
                                        <ReactMarkdown urlTransform={chuyenDiaChiAnh} components={customMarkdownComponents} remarkPlugins={[remarkPhysics, remarkBreaks]} rehypePlugins={[rehypeKatex]}>
