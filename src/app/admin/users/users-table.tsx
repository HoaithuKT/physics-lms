    ...
    <div>
      ...
      <div className="p-4 border-b flex justify-between items-center bg-gray-50">
        <input 
          ...
          className="px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 min-w-[300px]"
        />
                  ...
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      ...
                      className={`inline-flex items-center justify-center px-3 py-1.5 border border-transparent text-xs font-medium rounded shadow-sm text-white ${
                        user.is_active ? 'bg-red-600 hover:bg-red-700' : 'bg-orange-600 hover:bg-orange-700'
                      }`}
                    >
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <select value={newUser.course_id || ''} onChange={e => setNewUser({...newUser, course_id: e.target.value})} className="w-full border p-2 rounded text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-orange-500 focus:outline-none transition-colors">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl relative">
            ...
            <div className="space-y-4">
              <div>
                ...
                <a href="/api/admin/import-students/template" download className="text-xs text-orange-600 hover:underline block mt-2">Tải file Excel mẫu</a>
              </div>
              <div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button onClick={handleImport} disabled={importing} className="px-4 py-2 bg-orange-600 text-white rounded">Nhập</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-xl relative my-8">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold mb-1">Gán Lớp học thêm / Khóa học (LMS)</label>
                  <select value={userToEdit.course_id || ''} onChange={e => setUserToEdit({...userToEdit, course_id: e.target.value})} className="w-full border p-2 rounded text-sm bg-gray-50 focus:bg-white focus:ring-2 focus:ring-orange-500 focus:outline-none transition-colors">
                    <option value="">-- Chưa liên kết Khóa học --</option>
                    {courses.map(c => (
                      <option key={c.id} value={c.id}>{c.title}</option>
                    ))}
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
                ...
                <button type="submit" disabled={isSaving} className="px-4 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700">
