  ...
  const loadCandidates = async (matrixItems: MatrixItemDraft[], gradeVal: string, subjectVal: string) => {
    ...
    try {
      ...
      for (const item of matrixItems) {
        // Supabase/PostgREST mặc định chỉ trả tối đa 1000 dòng một lần. Dạng vật lý nào
        ...
        for (let from = 0; ; from += PAGE_SIZE) {
           ...
           *
           * Tên dạng không duy nhất trong kho. Bản cũ chỉ lọc theo tên dạng nên một dòng
           * ma trận thuộc Chương 1 vẫn rút được câu của Chương 5, đề ra sai chương mà
           * không ai biết. Kho Toán đo được 599 ô kho báo sai số câu, thổi phồng 4814 câu.
           * Ma trận lưu từ trước chưa có hai trường này thì bỏ qua, để đề cũ vẫn mở được.
           */
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      {step === 'select' ? (
        <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
          {lines.length === 0 && (
            <div className="text-center text-gray-400 py-20">Không có dạng vật lý nào trong ma trận.</div>
          )}
